...
import {
  compareDateTimeToday,
  compareDateTimeUSToday,
} from "../../utils/dateTime";
...
const getFileTypeIcon = (fileUrl) => {
  ...
  switch (fileExtension) {
    case ".pdf":
      return (
        <img
          src=" https://chat.zalo.me/assets/icon-pdf.53e522c77f7bb0de2eb682fe4a39acc3.svg"
          alt="PDF Icon"
        />
      );
    case ".doc":
...
const TextMsg = ({ el, menu, text }) => {
  ...
  const [anchorEl, setAnchorEl] = React.useState(null);
  const isSentByCurrentUser = el.from === user_id && el.text === text; 
  ...
  // const [messageStatus, setMessageStatus] = React.useState(isSentByCurrentUser ? "sent" : ""); 

  // React.useEffect(() => {
  //   const handleSeen = ({ userId, conversationId }) => {
  //     if (room_id === conversationId && userId === user_id) { // Check if the message is sent by the current user
        ...
        {reaction && (
          <Box
            sx={{
              ...
              left: incoming
                ? `calc(100% - ${Math.min(el.text.length * 10.5, 30)}px)`
                : "auto",
              right: incoming
                ? "auto"
                : `calc(100% - ${Math.min(el.text.length * 10, 100)}px)`,
              transform: "translateY(-50%)",
            }}
          >
            {reaction === "love" && (
        ...
        >
          ...
          <Tooltip title="Dowload" placement="top">
            ...
            >
              ...
              {/* Chỉ định kích thước nhỏ cho biểu tượng tải xuống */}
            </IconButton>
          </Tooltip>
        </Box>
      </Modal>
...
const VoiceMsg = ({ el, menu }) => {
  ...
  return (
    <Stack
      direction="row"
      justifyContent={el.from !== user_id ? "start" : "end"}
    >
      {el.from !== user_id && (
        <Avatar
          src={el.avatar}
      ...
      >
        ...
        <IconButton sx={{ width: 48, height: 48 }}>
          {" "}
          {/* Đặt chiều rộng và chiều cao của IconButton */}
          <GraphicEq sx={{ fontSize: 50 }} />{" "}
          {/* Đặt kích thước của biểu tượng GraphicEq */}
        </IconButton>
          ...
          <Tooltip title={timeLabel} placement={incoming ? "right" : "left"}>
            ...
            >
              ...
              <IconButton href={el.text} download>
                <DownloadSimple color="black" />{" "}
                {/* Chuyển màu biểu tượng tải xuống thành trắng */}
              </IconButton>
            </Stack>
          </Tooltip>
        </Stack>
    ...
    <Stack direction="row" justifyContent={incoming ? "start" : "end"}>
      ...
      >
        <Typography
          variant="body2"
          color={
            incoming ? theme.palette.text.primary : theme.palette.common.white
          }
        >
          <a
            href={el.link}
