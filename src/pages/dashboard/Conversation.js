...
const Conversation = ({ isMobile, menu, loadedMessagesCount, loading }) => {
  ...
  const messageFrom = tenMessages.filter((msg) => msg.from === user_id);
          ...
          <React.Fragment key={idx}>
            {(() => {
              switch (el.type) {
                ...
                case "Text":
                  // tin nhắn văn bản
                  return <TextMsg el={el} menu={menu} text= {text}/>;
                case "Image":
                  // hình ảnh
...
const ChatComponent = () => {
  ...
  useEffect(() => {
    ...
    let timeoutId;
    const lastMessage = current_messages[current_messages.length - 1];
    if (lastMessage && lastMessage.from === user_id) {
      setMessageStatus("sent");
      timeoutId = setTimeout(() => setMessageStatus("received"), 3000); // Update after 3 seconds
    }
  ...
  useEffect(() => {
    socket.on("seen", ({ userId, conversationId }) => {
      if (
        ...
        userId !== current_conversation?.user_id &&
        current_messages.length > 0 &&
        current_messages[current_messages.length - 1].from === user_id
      ) {
  ...
  }, [room_id, current_conversation?.user_id, user_id, current_messages]);
